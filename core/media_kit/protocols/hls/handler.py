...
from .config import HlsConfig, hls_config
...
class HlsStream(BaseStream):
    ...
    async def _start_pulling(self) -> bool:
        ...
        try:
            # 构建OpenCV捕获参数
            # HLS流通常是基于HTTP的，所以这里设置一些HTTP相关参数
            cap_params = [
    ...
    async def _pull_stream_task(self) -> None:
        ...
        try:
            ...
            exception_logger.exception(f"HLS流 {self._stream_id} 拉流任务异常: {str(e)}")
    ...
    async def _reconnect(self) -> bool:
        ...
        if self._retry_count >= self._max_retries:
            normal_logger.error(f"HLS流 {self._stream_id} 重试次数已达上限 ({self._max_retries}次)")
        ...
        try:
            ...
            exception_logger.exception(f"HLS流 {self._stream_id} 重连异常: {str(e)}")
