...
class RtspStream(BaseStream):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        try:
            ...
            # 从统一配置加载
            self._rtsp_config.retry_count = settings.PROTOCOLS.retry_count
            self._rtsp_config.retry_interval = settings.PROTOCOLS.retry_interval
            self._rtsp_config.timeout = settings.PROTOCOLS.timeout
            self._rtsp_config.port = settings.PROTOCOLS.rtsp_port
            self._rtsp_config.ssl_port = settings.PROTOCOLS.rtsp_ssl_port
            self._rtsp_config.auth_enable = settings.PROTOCOLS.rtsp_auth_enable
            self._rtsp_config.auth_user = settings.PROTOCOLS.rtsp_auth_user
            self._rtsp_config.auth_password = settings.PROTOCOLS.rtsp_auth_password
            self._rtsp_config.rtp_type = settings.PROTOCOLS.rtsp_rtp_type
            self._rtsp_config.max_buffer_ms = settings.PROTOCOLS.rtsp_max_buffer_ms
        except ImportError:
            # 如果无法导入配置，使用默认值
            self._rtsp_config = RtspConfig()

        # 如果配置中有RTSP特有配置，更新（优先级更高）
        rtsp_extra = config.get("rtsp", {})
        if rtsp_extra:
            for key, value in rtsp_extra.items():
                if hasattr(self._rtsp_config, key):
                    setattr(self._rtsp_config, key, value)
