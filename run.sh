...
start_service() {
    ...
    while true; do
        ...
        uvicorn app:app --host 0.0.0.0 --port 8002 --workers 1 --log-level warning >> $LOG_FILE 2>&1
...
check_port() {
    if command -v lsof &> /dev/null; then
        if lsof -i :8002 &> /dev/null; then
            echo "警告: 端口8002已被占用，尝试关闭现有进程..." | tee -a $ERROR_LOG
            kill $(lsof -t -i :8002) 2>/dev/null
            sleep 2
            if lsof -i :8002 &> /dev/null; then
                echo "错误: 无法释放端口8002，请手动关闭占用该端口的进程" | tee -a $ERROR_LOG
                return 1
            fi
        fi
    fi
