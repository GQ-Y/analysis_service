...
def process_stream_worker(task_id: str, stream_config: Dict, stop_event: Event, result_queue: Queue):
    ...
    try:
        ...
        if not stream_url:
            # 尝试从旧格式中获取
            source_config = stream_config.get("source", {})
            if isinstance(source_config, dict) and "urls" in source_config and source_config["urls"]:
                stream_url = source_config["urls"][0]
