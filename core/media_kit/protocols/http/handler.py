...
class HttpStream(BaseStream):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        try:
            ...
            # 从统一配置加载
            self._http_config.retry_count = settings.PROTOCOLS.retry_count
            self._http_config.retry_interval = settings.PROTOCOLS.retry_interval
            self._http_config.timeout = settings.PROTOCOLS.timeout
            self._http_config.user_agent = settings.PROTOCOLS.http_user_agent
            self._http_config.auth_enable = settings.PROTOCOLS.http_auth_enable
            self._http_config.auth_user = settings.PROTOCOLS.http_auth_user
            self._http_config.auth_password = settings.PROTOCOLS.http_auth_password
            self._http_config.use_cache = settings.PROTOCOLS.http_use_cache
            self._http_config.cache_timeout = settings.PROTOCOLS.http_cache_timeout
        except ImportError:
            # 如果无法导入配置，使用默认值
            self._http_config = HttpConfig()

        # 如果配置中有HTTP特有配置，更新（优先级更高）
        http_extra = config.get("http", {})
        if http_extra:
            for key, value in http_extra.items():
                if hasattr(self._http_config, key):
                    setattr(self._http_config, key, value)
