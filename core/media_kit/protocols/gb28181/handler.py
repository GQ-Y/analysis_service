...
class Gb28181Stream(BaseStream):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # GB28181特有配置
        self._gb28181_config = gb28181_config.from_dict({})
        
        # 如果配置中有GB28181特有配置，更新
        gb28181_extra = config.get("gb28181", {})
        if gb28181_extra:
            self._gb28181_config = gb28181_config.from_dict(gb28181_extra)
        
        # 解析设备信息
        self._parse_device_info(config)
        
        # ZLM相关配置
        self._zlm_host = config.get("zlm_host", "127.0.0.1")
        self._zlm_http_port = config.get("zlm_http_port", 8088)
        self._zlm_secret = config.get("zlm_secret", "")
        self._zlm_rtp_port = config.get("zlm_rtp_port", 0)  # 0表示自动选择
