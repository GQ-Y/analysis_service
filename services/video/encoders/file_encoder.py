...
class FileEncoder(BaseEncoder):
    ...
    def _encoding_thread(self, task_id: str, encoding_id: str, output_dir: str, output_path: str,
        ...
        try:
            ...
            normal_logger.info(f"计算得出最优比特率: {bitrate}k")
            ...
            try:
                ...
                # 获取智能帧丢弃器
                frame_dropper = self.frame_droppers.get(task_id)
                ...
                while task_id in self.encoding_tasks:
                    ...
                    # 智能帧丢弃检查
                    if frame_dropper and not frame_dropper.should_encode_frame():
                        # 跳过这一帧，继续下一帧
                        continue
                    ...
                    if frame_count % 100 == 0:
                        ...
                        if frame_dropper:
                            stats = frame_dropper.get_stats()
                            normal_logger.info(f"帧丢弃统计 - 总帧数: {stats['total_frames']}, 丢弃帧数: {stats['dropped_frames']}, 丢弃率: {stats['drop_rate']:.2%}, 有效FPS: {stats['effective_fps']:.1f}")
                    ...
                    if current_frame is None:
                        ...
                        if error_count > max_errors:
                            ...
                            # 创建默认帧
                            current_frame = self.create_default_frame(
                                final_width, 
                                final_height, 
